...
## calculate h2*var(y) on the observed 1/0 scale
sigmau2 = function(df){
	K = mean(df$y)
	Kgr = mean(df$y2[df$y])
	x_pop = qnorm(1-K)
...
df = simulate(5000, ngenes, maf, 0, sigmab, sigmae, qnorm(1-0.10), 0.05)
betalin = betalog = NULL
for(c in 5:ncol(df)){
	betalin = c(betalin, coef(lm(df$l ~ df[,c]))[2])
	betalog = c(betalog, coef(glm(df$y ~ df[,c]))[2])
}
coef(lm(betalog ~ betalin))[2]
dnorm(qnorm(1-mean(df$y)))
sd(betalog)
